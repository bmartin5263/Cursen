...
const void Tetromino::placeOnto(Tetromino::Board& board, cursen::Vect2 position) const
{
    for (auto& pair : blocks)
    {
        board[position.y + pair.y][position.x + pair.x] = graphic;
    }
}

const void Tetromino::removeFrom(Tetromino::Board& board, cursen::Vect2 position, chtype clear_value) const
{
    for (auto& pair : blocks)
    {
